...
Follow the instructions in `Continuous_Control.py` to get started with training your own agent. In particular, everything is arranged so that you provide a folder name where the results will be saved and that's it. Progress is shown every two seconds by default and the agent's progress is saved (by default) every minute - this includes performance curves to make it easy to compare different models.

In `misc.py` you'll find useful functions for loading data and plotting it. Examples can be found in `Report_Helper.ipynb`.

A smart agent can be watched by running `Watch_Pretrained_Agent.py`.
<br><br>
### Room to improve:
- Currently the code relies on defaults set in `ddpg_agent.py` for things like learning rates, network architecture, and actually most things. Choosing between regular and batchnorm'ed networks has to be done by commenting and un-commenting code in `model.py`. My usage was to include as many of these parameters as possible in the folder name.
